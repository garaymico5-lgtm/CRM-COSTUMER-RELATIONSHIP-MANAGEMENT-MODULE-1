KMJC GROUP:
BSCS 1-B 
-

            MICO ANGELO D. GARAY
            KIM DUPA
            CYRUS MANGUBAT
            JHON CHRIS JANOHAN






Title & Description: COSTUMER RELATIONSHIP MANAGEMENT (CRM) GOOD FOR SMALL ENTERPRISES
-
- This project is a Customer Relationship Management (CRM) system designed specifically for small businesses. It helps manage customer information, track interactions, and improve communication between the business and its clients. The system aims to simplify daily operations, organize data efficiently, and support better decision-making. By using this CRM, small enterprises can enhance customer satisfaction, increase productivity, and build stronger relationships with their customers.

Prerequisites:
-


               Python

               Flask  3.1.3

               Visual Studio Code

               Web Browser 

               GitHub

               git 2.53.0

Installation:
-

              1. Clone The repository:
                git clone https://github.com/your-username/your-repo-name.git
                cd your-repo-name

              2.install flask:
              pip install Flask == 3.1.3

              3.run the application:
              http://127.0.0.1:

              4. cloning the folder in vscode to repository using git:
              git init
              git add .
              git commit -am ""
              git remote add origin "link of your repository"
              git remote -v


Usage:
-

After running the application, open your web browser and go to: http://127.0.0.1:/50000, From there, you can  View the homepage of the CRM system Add new customer information, View and manage existing customers, Update customer details, Delete customer records. Use the navigation options in the system to move between different features and manage customer data efficiently.

Module: 
-

In my Module 1, I made a small changes to our project.In the Add menu you can see there an ADD ID, USERNAME, And EMAIL if you if your input is all correct there is a notification stated "Customer added successfuly".I moved the edit and delete menu into the “View Customer” section so we don’t have to keep going back.You can also see a Search customer if you have so many customer you just have to search their ID then it will search instantly. I haven’t designed the HTML yet so my group members can understand it easily and won’t get confused. We also tried checking the Module 1 we created, and so far there are no errors. This is the update for our Module 1.


In My module 2, I added some menus; add_interactions,view_interaction,update_interaction,delete_interaction.some have changes also in Module 2 we just put the update and delete interaction in the view_interaction so that it will not be time consuming,and since we added module 2 maybe  y port or my link broke and right now i still couldn't fix the problem so far so good about my HTML in my module 2 i just run immediately my html to the vscode if there is an error just my main problem here is how to fix my port because my port or flask is broken 

In My module 3, It is all about creating ticket and viewing ticket and this it will help the manager to easily determine or make a fair treatment to the customer we put TICKET ID so that no one will gonna feel treatment its first come first serve module inside the creating tickit is when the customer wants to reserve or buy the item they will go to this method so that the manager or seller won't confuse and in the view ticket you will see the tickets and there are some menus in the view tickets which is TICKET HISTORY, UPDATE TICKET STATUS, UPDATE TICKET PRIORITY,AND DELETE TICKET.

In my Module 4 i added the add_user,view_user,edit,delete,select_role, the purpose of this module is if you want to make an another account for the new employee, ony the admin will make an account for the employee its either admin,staff,support i just put the edt and delete user to the view_user so that the admin will not consume more time to press another menu.


In module 5 I put Reports so that i can identify which interaction hast the most uses and it is very important to us in this module because if we want to click the persons report we just go to customer report and it will count how many report of the customer make in month then we also add the ticket summary  so that we can identify  how many tickets were make within the month